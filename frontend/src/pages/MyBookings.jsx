...
const MyBookings = () => {
    ...
    // Function to calculate total price based on number of candidates and slot price
    const calculateTotalPrice = (candidates, pricePerSlot) => {
        return candidates * pricePerSlot
    }
                ...
                {Bookings.map((item, index) => (
                    <div key={index} className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-4 border-b'>
                        ...
                        <div className='flex-1 text-sm text-[#5E5E5E]'>
                            ...
                            {/* <p className='mt-1'><span className='text-sm text-[#3C3C3C] font-medium'>Total Price:</span> Rs {calculateTotalPrice(item.candidates, item.pricePerSlot)}</p> */}
                        </div>
                        <div className='flex flex-col gap-2 justify-end text-sm text-center'>
                            ...
                            {item.cancelled && !item.isCompleted && <button className='sm:min-w-48 py-2 border rounded text-[#696969] bg-red-200'>Cancelled</button>}
                        </div>
                    </div>
                ))}
            </div>
